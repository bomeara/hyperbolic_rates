...
#sim_results_processed_rep1 <- subset(sim_results_processed, replicate_id==1 & deltaAIC==0)
sim_results_processed_rep1 <- subset(sim_results_processed, deltaAIC==0)
...
#ggcool <- ggplot(sim_results_processed_rep1, aes(x=(time), y=(empirical_rate), color=data_size)) + theme_bw() + xlab("Time") + ylab("Rate") + ggtitle("Simulated data") + theme(text=element_text(family="Helvetica"))  + geom_line(aes(y=predicted_rate_no_mserr)) + geom_line(aes(y=(generating_noiseless_rate)), color="black", linetype="dotted") + facet_grid(error_sd ~ scenario + data_size, scales="free") + scale_x_continuous(trans="log10") + scale_y_continuous(trans="log10") 
#print(ggcool)

ggcool <- ggplot(sim_results_processed_rep1, aes(x=(time), y=(empirical_rate), color=data_size)) + geom_point(alpha=0.1, color="gray", shape=20) + theme_bw() + scale_color_viridis(discrete=TRUE, option="viridis", begin=0.2, end=0.9) + xlab("Time") + ylab("Rate") + ggtitle("Simulated data hmb approach") + theme(text=element_text(family="Helvetica"))  + geom_line(aes(y=predicted_rate_no_mserr, group=replicate_id)) + geom_line(aes(y=(generating_noiseless_rate)), color="red") + facet_grid(error_sd ~ scenario + data_size) + scale_x_continuous(trans="log10") + scale_y_continuous(trans="log10") 
print(ggcool)
ggsave(filename="FigureS11.pdf", plot=ggcool, width=12, height=14, units="in", dpi=300)
...
ggcool <- ggplot(sim_results_processed_rep1, aes(x=(time), y=(empirical_rate), color=data_size)) + theme_bw() + xlab("Time") + ylab("Rate") + ggtitle("Simulated data hmb") + theme(text=element_text(family="Helvetica")) + geom_line(aes(y=(generating_noiseless_rate)), color="black") + geom_point(aes(y=predicted_rate_no_mserr)) + facet_grid(rows=vars(error_sd), cols=vars(scenario), scales="free")
print(ggcool)
...
for (dataset_name in datasets) {
 ...
 # }
  ...
  local_dataset <- local_dataset |> tidyr::pivot_longer(cols=c("predicted_rate", "empirical_rate", "predicted_rate_no_mserr"), names_to="rate_type", values_to="rate")
	...
	# # no h
	# 	gcool <- ggplot(local_dataset, aes(x=time, y=log(rate))) + theme(text=element_text(family="Helvetica"))+ geom_point(aes(x=time, y=log(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) + 
	# 	geom_ribbon(data=x_no_h, aes(ymin=constant_component_proportion_ribbon_lower, ymax=constant_component_proportion_ribbon_upper), fill=fill_colors[6], alpha=0.7) + 
	# 	geom_ribbon(data=x_no_h, aes(ymin=linear_component_proportion_ribbon_lower, ymax=linear_component_proportion_ribbon_upper), fill='cadetblue2', alpha=0.7) + 
	# 	geom_ribbon(data=x_no_h, aes(ymin=hyperbolic_component_proportion_ribbon_lower, ymax=hyperbolic_component_proportion_ribbon_upper), fill=fill_colors[4], alpha=0.7) 
	...
	# 	print(ylim_setting)
	# 	gcool <- gcool + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
	# 	geom_line(data=x_no_h, aes(x=time, y=log(predicted_rate_no_h)), color="black") +
	# 	geom_line(data=x_no_h, aes(x=time, y=log(predicted_rate_no_h_lower)), color="black", lty="dashed") +
	# geom_line(data=x_no_h, aes(x=time, y=log(predicted_rate_no_h_upper)), color="black", lty="dashed") + theme(panel.border = element_blank())
	# 	all_plots[[length(all_plots)+1]] <- gcool	
	# }
	...
	# 	gcool <- ggplot(x_no_h, aes(x=time, y=predicted_rate_no_h)) + theme(text=element_text(family="Helvetica")) + geom_line(color="black") + geom_line(aes(y=predicted_rate_no_h_upper), color="black", linetype="dashed") + geom_line(aes(y=predicted_rate_no_h_lower), color="black", linetype="dashed") + theme(panel.border = element_blank())  +  theme_bw() + xlab("Time (MY)") + ylab("Rate") + ylim(c(0, NA)) + scale_x_continuous(trans="log10") + scale_y_continuous(trans="log10")
	# 	all_plots[[length(all_plots)+1]] <- gcool
...
g <- cowplot::plot_grid(plotlist=all_plots, ncol=4, labels = "AUTO", byrow=TRUE)
print(g)
ggsave(filename="Figure1.pdf", plot=g, width=12, height=14, units="in", dpi=300)
...
## cowplot new approach

```{r coloronlycowplottry2, fig.width=12, fig.height=18, eval=TRUE, error=FALSE, warning=FALSE, message=FALSE}
library(scales)
datasets_and_n <- hyperr8_analysis |> dplyr::distinct(dataset, n) |> dplyr::arrange(desc(n))
datasets <- unique(datasets_and_n$dataset)

repsets <- c("Original", paste0("Rep ", sequence(10)))
focal_dataset <- data.frame()
...
for (dataset_index in sequence(length(datasets))) {
 ...
 # }
  ...
  ribbon_dataset$scaling_factor <- apply(ribbon_dataset[,c("scaling_factor_original", "scaling_factor_slope")], 1, max)
  ribbon_dataset$scaling_factor <- sapply(ribbon_dataset$scaling_factor, min, 3.5) # handles ginormous values
  local_dataset$empirical_rate_minus_hyperbolic <- local_dataset$empirical_rate - (local_dataset$param_h/local_dataset$time)
  
  #print(tail(local_dataset[,c("time", "empirical_rate", "empirical_rate_minus_hyperbolic", "param_h", "param_m", "param_b")]))
  
  save(local_dataset, file=paste0("~/Downloads/local_dataset_", dataset_name, "_", rep_name, ".rda"))
  ...
  local_dataset <- local_dataset |> tidyr::pivot_longer(cols=c("predicted_rate", "empirical_rate", "predicted_rate_no_mserr"), names_to="rate_type", values_to="rate")
	...
	if(rep_name=="Original") {
		#ylim_setting_no_h <- c(NA, 10+log(max(range_dataset_original$empirical_rate, na.rm=TRUE)))
		# Original rates
		gcool <- ggplot(local_dataset, aes(x=time, y=log(rate))) + theme(text=element_text(family="Helvetica")) + geom_point(aes(x=time, y=log(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) +  theme(text=element_text(family="Helvetica")) + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
		geom_line(data=x_no_h, aes(y=log(predicted_rate_no_h)), color="orange") + geom_line(data=x_no_h, aes( y=predicted_rate_no_h_upper), color="orange", linetype="dashed") + geom_line(data=x_no_h, aes( y=log(predicted_rate_no_h_lower)), color="orange", linetype="dashed") + 
		geom_line(data=x, aes(y=log(predicted_rate)), color="blue") + geom_line(data=x, aes( y=predicted_rate_upper), color="blue", linetype="dashed") + geom_line(data=x, aes( y=log(predicted_rate_lower)), color="blue", linetype="dashed") + 
		...
		# gcool <- ggplot(local_dataset, aes(x=time, y=log(empirical_rate_minus_hyperbolic))) + theme(text=element_text(family="Helvetica")) + geom_point(aes(x=time, y=log(empirical_rate_minus_hyperbolic)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) +  theme(text=element_text(family="Helvetica")) + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
		# geom_line(data=x_no_h, aes(y=log(predicted_rate_no_h)), color="orange") + geom_line(data=x_no_h, aes( y=predicted_rate_no_h_upper), color="orange", linetype="dashed") + geom_line(data=x_no_h, aes( y=log(predicted_rate_no_h_lower)), color="orange", linetype="dashed") + 	
		# theme(panel.border = element_blank())  +  theme_bw() + xlab("Time (MY)") + ylab("Rate") + ylim(c(0, NA))
		...
		if(rep_name==repsets[length(repsets)]) {
			padding <- 0.002
			local_dataset <- subset(all_ribbon_data, dataset==dataset_name)
			narrow_hyperbolic <- which(local_dataset$hyperbolic_component_proportion_upper-local_dataset$hyperbolic_component_proportion_lower < padding)
			narrow_linear <- which(local_dataset$linear_component_proportion_upper-local_dataset$linear_component_proportion_lower < padding)
...
#g <- cowplot::plot_grid(plotlist=all_plots, nrow=4, labels = "AUTO", byrow=FALSE, rel_widths=rep(1, 2), rel_heights=rep(1, 2))
g <- cowplot::plot_grid(plotlist=all_plots, ncol=2, labels = "AUTO", byrow=TRUE)
print(g)
ggsave(filename="Figure1New.pdf", plot=g, width=12, height=14, units="in", dpi=300)
#all_ribbon_data$dataset <- factor(all_ribbon_data$dataset, levels=datasets)
...
print(g)
ggsave(filename="RegressionComparison.pdf", plot=g, width=16, height=14, units="in", dpi=300)
```
...
```{r cowplotdetrended, fig.width=12, fig.height=18, eval=FALSE, error=FALSE, warning=FALSE, message=FALSE}


log_handle_negs <- function(x) {
	logx <- log(x)	
	logx[is.na(logx)] <- log(-x[is.na(logx)])
	return(logx)
}
...
for (dataset_index in sequence(length(datasets))) {
 ...
 # }
  ...
  local_dataset <- local_dataset |> tidyr::pivot_longer(cols=c("predicted_rate", "empirical_rate", "predicted_rate_no_mserr"), names_to="rate_type", values_to="rate")
	...
	starting_values <- log_handle_negs(x$predicted_rate)-0.5*x$scaling_factor
	x$constant_component_proportion_ribbon_lower <- starting_values
	x$constant_component_proportion_ribbon_upper <- x$constant_component_proportion_ribbon_lower+x$scaling_factor*(x$constant_component_proportion)
	x$linear_component_proportion_ribbon_lower <- x$constant_component_proportion_ribbon_upper
	x$linear_component_proportion_ribbon_upper <- x$linear_component_proportion_ribbon_lower + x$scaling_factor*(x$linear_component_proportion)
	...
	x_no_h <- ribbon_dataset
	starting_values_no_h <- log_handle_negs(x_no_h$predicted_rate_no_h)-0.5*x_no_h$scaling_factor
	x_no_h$constant_component_proportion_ribbon_lower <- starting_values_no_h
	x_no_h$constant_component_proportion_ribbon_upper <- x_no_h$constant_component_proportion_ribbon_lower+x_no_h$scaling_factor*(x_no_h$constant_component_proportion)
	x_no_h$linear_component_proportion_ribbon_lower <- x_no_h$constant_component_proportion_ribbon_upper
	x_no_h$linear_component_proportion_ribbon_upper <- x_no_h$linear_component_proportion_ribbon_lower + x_no_h$scaling_factor*(x_no_h$linear_component_proportion)
	...
	if(rep_name=="Original") {
		#ylim_setting_no_h <- c(NA, 10+log(max(range_dataset_original$empirical_rate, na.rm=TRUE)))
		# Original rates
		gcool <- ggplot(local_dataset, aes(x=time, y=log_handle_negs(rate))) + theme(text=element_text(family="Helvetica")) + geom_point(aes(x=time, y=log_handle_negs(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) +  theme(text=element_text(family="Helvetica")) + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
		geom_line(data=x_no_h, aes(y=log_handle_negs(predicted_rate_no_h)), color="orange") + geom_line(data=x_no_h, aes( y=log_handle_negs(predicted_rate_no_h_upper)), color="orange", linetype="dashed") + geom_line(data=x_no_h, aes( y=log_handle_negs(predicted_rate_no_h_lower)), color="orange", linetype="dashed") + 
		geom_line(data=x, aes(y=log_handle_negs(predicted_rate)), color="blue") + geom_line(data=x, aes( y=log_handle_negs(predicted_rate_upper)), color="blue", linetype="dashed") + geom_line(data=x, aes( y=log_handle_negs(predicted_rate_lower)), color="blue", linetype="dashed") + 
	
		theme(panel.border = element_blank())  +  theme_bw() + xlab("Time (MY)") + ylab("Rate") 
		gcool <- gcool + scale_x_continuous(trans="log10", labels=label_comma(), n.breaks=4)
		...
		gcool <- ggplot(local_dataset, aes(x=time, y=log_handle_negs(empirical_rate_minus_hyperbolic))) + theme(text=element_text(family="Helvetica")) + geom_point(aes(x=time, y=log_handle_negs(empirical_rate_minus_hyperbolic)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) +  theme(text=element_text(family="Helvetica")) + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
		geom_line(data=x_no_h, aes(y=log_handle_negs(predicted_rate_no_h)), color="orange") + geom_line(data=x_no_h, aes( y=log_handle_negs(predicted_rate_no_h_upper)), color="orange", linetype="dashed") + geom_line(data=x_no_h, aes( y=log_handle_negs(predicted_rate_no_h_lower)), color="orange", linetype="dashed") + 	
		theme(panel.border = element_blank())  +  theme_bw() + xlab("Time (MY)") + ylab("Rate") 
...
print(g)
ggsave(filename="Figure1Detrended.pdf", plot=g, width=12, height=14, units="in", dpi=300)
#all_ribbon_data$dataset <- factor(all_ribbon_data$dataset, levels=datasets)
...
```{r coloronlycowplotnomserr, fig.width=12, fig.height=18, eval=FALSE, error=FALSE, warning=FALSE, message=FALSE}
library(scales)
datasets_and_n <- hyperr8_analysis |> dplyr::distinct(dataset, n) |> dplyr::arrange(desc(n))
datasets <- unique(datasets_and_n$dataset)
...
for (dataset_name in datasets) {
 ...
 # }
  ...
  local_dataset <- local_dataset |> tidyr::pivot_longer(cols=c("predicted_rate", "empirical_rate", "predicted_rate_no_mserr"), names_to="rate_type", values_to="rate")
	...
	starting_values <- log(x$predicted_rate_no_mserr)-0.5*x$scaling_factor
x$constant_component_proportion_ribbon_lower <- starting_values
	x$constant_component_proportion_ribbon_upper <- x$constant_component_proportion_ribbon_lower+x$scaling_factor*(x$constant_component_proportion)
	x$linear_component_proportion_ribbon_lower <- x$constant_component_proportion_ribbon_upper
	x$linear_component_proportion_ribbon_upper <- x$linear_component_proportion_ribbon_lower + x$scaling_factor*(x$linear_component_proportion)
	x$hyperbolic_component_proportion_ribbon_lower <- x$linear_component_proportion_ribbon_upper 
	x$hyperbolic_component_proportion_ribbon_upper <- x$hyperbolic_component_proportion_ribbon_lower + x$scaling_factor*(x$hyperbolic_component_proportion)
	x$rate <- x$predicted_rate_no_mserr
	...
	gcool <- ggplot(local_dataset, aes(x=time, y=log(rate))) + theme(text=element_text(family="Helvetica"))+ geom_point(aes(x=time, y=log(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) + 
		...
		print(ylim_setting)
		gcool <- gcool + theme_bw() + xlab("Time (MY)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
		geom_line(data=x, aes(x=time, y=log(predicted_rate_no_mserr)), color="black") +
...
g <- cowplot::plot_grid(plotlist=all_plots, ncol=3, labels = "AUTO", byrow=TRUE)
print(g)
ggsave(filename="Figure1_nomserr.pdf", plot=g, width=12, height=14, units="in", dpi=300)
...
```{r, fig.width=12, fig.height=10, eval=FALSE, error=FALSE, warning=FALSE, message=FALSE}
library(scales)
...
x$constant_component_proportion_ribbon_lower <- starting_values
	...
	gcool <- ggplot(local_dataset, aes(x=time, y=log(rate))) + geom_point(aes(x=time, y=log(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) + theme(text=element_text(family="Helvetica")) +
		geom_ribbon(data=x, aes(ymin=constant_component_proportion_ribbon_lower, ymax=constant_component_proportion_ribbon_upper), fill=fill_colors[6], alpha=0.7) + 
		geom_ribbon(data=x, aes(ymin=linear_component_proportion_ribbon_lower, ymax=linear_component_proportion_ribbon_upper), fill='cadetblue2', alpha=0.7) + 
		geom_ribbon(data=x, aes(ymin=hyperbolic_component_proportion_ribbon_lower, ymax=hyperbolic_component_proportion_ribbon_upper), fill=fill_colors[4], alpha=0.7) 
...
g <- cowplot::plot_grid(plotlist=all_plots, ncol=3, labels = "AUTO", byrow=TRUE)
print(g)
ggsave(filename="Figure1_Funny.pdf", plot=g, width=12, height=12, units="in", dpi=300)
...
g <- ggplot(subset(all_r2_regular, model=="hmb"), aes(x=r2, color=comparison_type)) + theme(text=element_text(family="Helvetica")) + geom_line(stat="density", size = 1) + facet_wrap(~dataset, scales='free')
print(g)
```
...
for (i in sequence(nrow(comparisons_average))) {
 ...
 g <- ggboxplot(best_models, x="comparison_type", y="r2", color="comparison_type", facet.by="dataset", ylab="R^2", xlab="Comparison Type", ylim=c(0,1.1), legend="none") + ggprism::add_pvalue(comparisons_average, bracket.shorten=c(0.09), color="darkgray", braket.color="darkgray") + theme(legend.position="none") + scale_y_continuous(breaks=c(0, 0.25, 0.5, 0.75, 1), labels=c(0, 0.25, 0.5, 0.75, 1))
  print(g)
  ggsave(filename="Figure2.pdf", plot=g, width=12, height=9, units="in", dpi=300)
