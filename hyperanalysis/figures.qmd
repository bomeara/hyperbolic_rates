...
print(ggcool)
ggsave(filename="FigureS11.tiff", plot=ggcool, width=12, height=14, units="in", dpi=300, compression = "lzw")
...
print(ggcool)
ggsave(filename="FigureS12.tiff", plot=ggcool, width=12, height=14, units="in", dpi=300, compression = "lzw")
...
print(ggcool)
ggsave(filename="FigureS13.tiff", plot=ggcool, width=12, height=14, units="in", dpi=300, compression = "lzw")
 ...
 # }
  ...
  local_dataset <- local_dataset |> tidyr::pivot_longer(cols=c("predicted_rate", "empirical_rate", "predicted_rate_no_mserr"), names_to="rate_type", values_to="rate")
	...
	gcool <- ggplot(local_dataset, aes(x=time, y=log(rate))) + theme(text=element_text(family="Helvetica"))+ geom_point(aes(x=time, y=log(rate)), colour="gray42", alpha=max(0.05, min(1, 10/sqrt(nrow(local_dataset)) )), size=0.8) + 
		...
		#print(ylim_setting)
		gcool <- gcool + theme_bw() + xlab("Time (Years)") + ylab("Rate") + ggtitle(gsub("Rep 1", "Randomized", paste0(dataset_name, ": ", unique(local_dataset$rep)))) +
...
print(g)
ggsave(filename="Figure1.tiff", plot=g, width=12, height=14, units="in", dpi=300, compression = "lzw")
...
print(g)
ggsave(filename="RegressionComparison.tiff", plot=g, width=16, height=14, units="in", dpi=300, compression = "lzw")
```
...
for (i in sequence(nrow(comparisons_average))) {
 ...
 g <- ggboxplot(best_models, x="comparison_type", y="r2", color="comparison_type", facet.by="dataset", ylab="R^2", xlab="Comparison Type", ylim=c(0,1.1), legend="none") + scale_color_viridis(discrete=TRUE, option="viridis", end=0.8) + ggprism::add_pvalue(comparisons_average, bracket.shorten=c(0.09), color="darkgray", braket.color="darkgray") + theme(legend.position="none") + scale_y_continuous(breaks=c(0, 0.25, 0.5, 0.75, 1), labels=c(0, 0.25, 0.5, 0.75, 1))
  print(g)
  ggsave(filename="Figure2.tiff", plot=g, width=12, height=9, units="in", dpi=300, compression = "lzw")
...
library(dentist)
library(ggplot2)
tiff(file="FigureS6_BarnoskyHMBDentist.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
plot(raw_info_for_dentist$`Barnosky et al. (2011)__hmb`$dentist)
dev.off()

tiff(file="FigureS3_GingerichHMBDentist.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
plot(raw_info_for_dentist$`Gingerich (1983)__hmb`$dentist)
dev.off()

tiff(file="FigureS4_HenaoDiazHM0Dentist.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
plot(raw_info_for_dentist$`Henao Diaz et al. (2019)__hm0`$dentist)
dev.off()

tiff(file="FigureS5_HoHMBDentist.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
plot(raw_info_for_dentist$`Ho et al. (2005)__hmb`$dentist)
dev.off()

tiff(file="FigureS1_PureBirthHMBDentist.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
plot(raw_info_for_dentist$`Pure birth simulation__hmb`$dentist)
dev.off()

tiff(file="FigureS2_UyedaHMB.tiff", width=12, height=9, units="in", res=300, compression = "lzw")
...
```{r funny, fig.width=12, fig.height=18, eval=TRUE, error=FALSE, warning=FALSE, message=FALSE}
tar_load(hyperr8_analysis_all)
hyperr8_analysis <- hyperr8_analysis_all
datasets_and_n <- hyperr8_analysis_all |> dplyr::distinct(dataset, n) |> dplyr::arrange(desc(n))
datasets <- unique(datasets_and_n$dataset)
...
g <- cowplot::plot_grid(plotlist=all_plots, ncol=3, labels = "AUTO", byrow=TRUE)
print(g)
ggsave(filename="FigureS7.tiff", plot=g, width=12, height=18, units="in", dpi=300, compression = "lzw")
